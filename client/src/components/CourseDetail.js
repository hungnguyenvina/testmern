...
import { loadSingleCourse,loadCourseCurriculum } from '../redux/actions/courseAction';
...
import $ from 'jquery';
import CourseDetailCss from './CourseDetail.css';
...
class CourseDetail extends Component {
  ...
  playVideo(lessionContent) {
    ...
    let media = ""; 
		...
		var indexM4V = lessionContent.endsWith("m4v");
                        ...
                        var indexOGG = lessionContent.endsWith("ogg");
                        if(indexMP4 > 0)
                        {
                            media = 
                            {
                      ...
                      if(media=="")
                      {
                        ...
                        $('#VideoPlayer').hide();
                        this.setState({dialogVisible: true,isVideoType: false,lessionContent:lessionContent});  
                      }
                      else{
                        console.log('media : '+media);
                        $('#VideoPlayer').show();
                        this.props.setMedia('VideoPlayer', media);
                        this.setState({dialogVisible: true,isVideoType: true});  
                        //alert($('.jp-media'));
                        $('.jp-media').attr("width","96%"); 
                      }               
  }
  ...
  render() {
    ...
    let updateLessionContent = renderHTML(this.state.lessionContent);
    ...
    return (
      ...
      <React.Fragment>
        ...
        <Dialog
                ...
                >
                  <Dialog.Body>
                  <div style={{display: this.state.isVideoType?'none':'inline-block',width:'95%',padding:'20px',textAlign:'left',height:'auto',minHeight:'300px',backgroundColor:'Lavender',boder:'1px solid Green'}}>{updateLessionContent}</div>
                    <VideoPlayer style={{display: this.state.isVideoType?'inline-block':'none'}} style={{width:'200px !important'}} id="VideoPlayer" m4vURL={course_picture_url} />
                  </Dialog.Body>

                  <Dialog.Footer style={{marginTop:'10px'}} className="dialog-footer">
...
const mapStateToProps = state => {
    return {
        single_course : state.course_reducer.single_course,
