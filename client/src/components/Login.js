...
import {Form,Input,Button,Layout,Alert} from 'element-react';
...
class Login extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.loginSuccess) {
      ...
      this.setState({
        showAlert:true,
        messageAlert: nextProps.message
      })
    }
  }
      ...
      <React.Fragment>
        <Layout.Row className="tac" style={{marginTop:'30px'}}>
          <Layout.Col xs="24" sm="24" md="24" lg="12" offset="6">
            <div style={{display:this.state.showAlert==true?'inline-block':'none', marginTop:'20px'}}>
          <Alert title={this.state.messageAlert} type="error" showIcon={true} />
          </div>
        <Form
          className="en-US"
